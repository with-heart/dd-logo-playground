...
export interface UrlSettingsValues {
  chroma: number
...
export interface UrlSettingsActions {
...
export type UrlSettings = UrlSettingsValues & UrlSettingsActions
...
export const useUrlSettings = (): UrlSettings => {
  ...
  const actions = useMemo<UrlSettingsActions>(
    () => ({
      randomizeColors: () => {
        const l = rand(0, 1)
        const c = rand(0, 0.4)
